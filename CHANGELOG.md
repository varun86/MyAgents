...
- **任务高级配置：单任务可覆盖 runtime / model / 权限模式 / MCP**：派发和编辑任务面板新增「高级配置」折叠区，可以为单个任务单独指定运行时（builtin / Claude Code / Codex / Gemini）、模型、权限模式（auto / fullAgency）、MCP 服务器列表，不影响工作区默认。Cron 触发的内置路径也尊重权限模式覆盖，无人值守任务在 permissionMode 未设置时默认按 fullAgency 跑，不再被第一个工具调用阻塞。
- **想法多选合并 / 删除**：想法卡支持多选后批量合并或删除。合并采用预校验 + 逐条 best-effort 删除 + 部分失败如实上报，不会再像旧实现那样在"删了第 1 条然后中途失败"时静默丢数据。
- **想法搜索关键词高亮**：搜索结果在文本里直接标出关键词命中位置，扫读更快。
- **AI 输入框 `@` 引用支持「想法」**：`@` picker 增加想法 tab，⌘/Ctrl+←/→ 在 tab 之间切换，回车直接把整条想法插入输入框；空态会回显当前查询关键词，避免"什么都没显示不知道为啥"。
- **任务卡 hover 多了「查看任务会话」按钮**：贴在 ⋯ 左边，单击直达该任务最近一次会话 Tab，省掉先点详情再切会话的两步。
- **派发任务时一并写 verify.md**：派发对话框新增可折叠的 verify.md 编辑区，建任务时直接落盘验收清单，省掉"先建后编辑加 checklist"的两步。
- **任务编辑模式开放三份文档**：原来 AI 对齐型任务的编辑面板只能改 task.md，现在 task.md 和 verify.md 都可编辑、progress.md 以只读预览呈现，跟详情面板看到的内容一致。
- **内嵌浏览器面板**：工作区工具栏新增地球图标，点击在分屏面板里打开嵌入式浏览器；地址栏 / 前进后退 / 刷新 / 在系统浏览器打开 / 关闭一应俱全。Cmd（macOS）/ Ctrl（Win/Linux）/ 中键点击链接会跳到系统默认浏览器，方便在不打断当前网页的情况下另开一处。空白态是设计过的暖纸底色，不再是系统白屏 about:blank。

### Fixed

- **Cron 日报时不时不送达 IM Bot（自 0.2.2 起的回归）**：定时任务跑完出报告后 IM Bot 偶发收不到——sidecar 内存里的事件队列是「取了就消失」的至多一次语义，AI 任何静默 / 超时 / HEARTBEAT_OK 都会让那条 cron 结果在已被取走但还没送出的窗口里彻底丢失；Rust 推送失败的 dedup 记录又会让下次心跳静默压制重投。整链改成 Rust 持有真相源（每个 bot 一个待投递 Cron 队列）+ 推送成功才清除 + 心跳级联唤醒，语义变成"会到，最差晚 30 分钟到"，不会再丢。
- **任务级 MCP override 配置后不生效**：高级配置勾了 MCP 服务器，cron 触发时仍然是 workspace 默认。修复后 sidecar 直接 abort + restart 立即应用，不再被防抖窗口和 snapshotted-session guard 拖到下下次心跳才生效。
- **多个 Cron tick 并发互相打断**：高频心跳遇上需要切 session / MCP 的 cron 任务时，会出现"上一任务还没切完下一任务又抢资源"的串扰。新增 dispatch 锁串行化 /cron/execute-sync 的整段临界区，杜绝跨 tick 状态污染。跟随 Agent 模式的 cron 每个 tick 也强制 reconcile 回 workspace 默认 MCP，杜绝前一任务的 override 跨 tick 泄漏。
- **聊天流式输出期间切 Tab 回来不再丢底部锚定 / 不再跳到对话顶部**：之前 AI 流式输出过程中切走再切回，Virtuoso 在隐藏期对零或陈旧布局的测量反馈会把"自动跟随底部"的锁误置成 false，有时还会让滚动停在历史顶部。现在切 Tab 期间 Virtuoso 的高度反馈被屏蔽，回到该 Tab 时若之前在底部跟随中会自动重新锚定。

### Improved

- **任务派发 / 详情 / 编辑三个面板视觉打通**：以前三处的标题字号、宽度、间距、Toggle 实现各走各的，给人"同一个任务三种语言"的感觉。现在共用 PanelChrome：宽度统一 780px、节区间距 28px、标题 18px、Toggle / footer / Esc 行为对齐。
- **任务卡 progress.md 长内容自动收纳**：长任务的 progress 经常涨到几千字撑爆卡片，默认折叠到 ~280px 高加渐隐遮罩，提供「展开全部 / 收起」按钮；短内容不显示折叠按钮。
- **任务卡活动栏改单行**：以前是两行带边框，长中文会把活动栏推到超过卡片宽度；现在单行截断 + 静音色，视觉更轻。
- **任务多选交互精简**：去掉了顶上「多选模式」横条，浮动菜单贴左，框选 / 取消选更顺手。
- **「会话详情」按钮换 icon + 文案 + 黑色 tooltip**：原本纯文字按钮容易被忽略。
