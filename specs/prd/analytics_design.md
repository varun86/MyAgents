...
| 事件名 | 触发时机 | params | 状态 |
|--------|----------|--------|------|
| `session_new` | 创建新会话 | `{ session_id, tab_id?, triggered_by, entry_intent, runtime, has_initial_message, agent_hash }` —— `triggered_by` 是 UI surface，`entry_intent` 是入口语义，`has_initial_message` 来自真实出生上下文；详见 `prd_0.2.36_analytics_entry_funnel_reliability.md` | ✅ 已实现 |
| `session_switch` | 切换到历史会话 | `{}` | ✅ 已实现 |
...
| `history_open` | 从启动页历史记录打开目标 session | `{ session_id, agent_hash, runtime }` | ✅ 已实现 |
...
| `session_switch` | `Chat.tsx` | 历史记录切换时 |
