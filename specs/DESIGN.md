...
**使用场景**：
- Launcher 页面：「Agent 工作区」「历史对话」（静态标题；历史标题行可吸顶并承载弱化筛选器）
...
**区域内边距**：24px (`px-6 py-6`)。

**Section 间距**：
| Section | 规则 |
|---------|------|
| Agent 工作区 | 顶部 section，默认展示 4 个可见工作区 |
| 分割线 | `border-t border-[var(--line-subtle)]`，位于工作区和历史之间，保持在右栏内容内距内，不贴左右边缘 |
| 历史对话 | 标题行 `sticky top-0`，吸顶时保持 `var(--paper)` 近似背景；标题行自身不加横穿整栏的底边 |

**入口边界**：
- Launcher 右栏不展示「我的任务」摘要。
- Launcher 右栏不展示「新建任务」按钮。
- 历史标题行不展示「全部 →」跳转，只保留搜索 icon 和工作区筛选器。

### 15.4 Agent 工作区

```
Header:
  - 标题: text-sm, font-semibold, tracking-[0.04em], var(--ink-muted)
  - 右侧: Logs（仅 dev tools）+ AddWorkspaceMenu
...
展开态:
  ...
  - 底部显示「收起」按钮
  - 展开 / 收起使用 max-height transition，尊重 motion-reduce
```
...
项目路径: 12px (text-xs), var(--ink-muted) / 70%

频道标签:
  - 与项目名称同一行
  - text-xs
  - 不换行；空间不足时在标题行内裁切，不新增第三行
