...
| 后端 | Node.js v24 + Claude Agent SDK 0.3.173（多实例 Sidecar） |
...
- **SDK 类型定义**：`node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts`（当前版本 0.3.173）
...
| 比较工作区路径（`Project.path` ↔ cron/task `workspacePath` / session `agentDir` / config `defaultWorkspacePath`）用 raw `===` 或 inline `.replace(/\\/g,'/')` | Win 上 `projects.json` 存反斜杠、cron/session 存正斜杠 → 永不相等且**静默**：#320 定时任务"升级为新版任务"全报"找不到工作区"，连带 task 卡掉工作区名 / Recent 会话空白 / 工作区过滤全"(已失效)" | `workspacePathsEqual(a,b)` 谓词 / `normalizeWorkspacePathIdentity(p)` 做 Set·Map 键（`src/shared/workspacePath.ts`，Rust `cron_task.rs::normalize_path` 的逐行 TS 端口；键 build+lookup 两侧都要过）。详见 `pit_of_success.md`「workspacePath」 | — (语义，靠 review) |
